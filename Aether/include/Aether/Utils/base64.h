// forked from https://github.com/tobiaslocker/base64
// Change By vo17245: Remove exception, and handle error by return value
...
inline bool to_base64(std::string_view data,std::string& buf) {
    buf=encode_into<std::string>(std::begin(data), std::end(data));
    return true;
}


template<class OutputBuffer>
inline bool decode_into(std::string_view data,OutputBuffer& decoded) {
...
inline  bool from_base64(std::string_view data,std::string& buf) {
    return decode_into<std::string>(data,buf);
}
