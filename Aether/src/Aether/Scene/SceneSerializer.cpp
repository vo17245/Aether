...
SceneSerializer::WriteMem(Scene& scene, const std::string& name,
                          const CameraStorage& camera)
{
...
std::optional<DeserializationResult>
SceneSerializer::LoadFile(const std::filesystem::path& path)
{
...
std::optional<json> GetFromObject(const json& json_obj,
...
static std::optional<CameraStorage>
Json2CameraStorage(const json& json_cameraStorage)
{
...
Json2TransformComponent(const json& json_TransformComponent)
{
...
std::optional<PointLightComponent>
Json2PointLightComponent(const json& json_plc)
{
...
std::optional<Ref<Scene>> Json2Scene(const json& json_scene)
{
    ...
    for (size_t i = 0; i < json_entities.size(); i++)
    {
        ...
        auto opt_json_TransformComponent =
            GetFromObject(json_entity, "TransformComponent");
        if (opt_json_TransformComponent)
        {
            const json& json_TransformComponent = opt_json_TransformComponent.value();
            auto opt_TransformComponent =
                Json2TransformComponent(json_TransformComponent);
            ...
            entity.AddComponent<TransformComponent>(
                std::move(opt_TransformComponent.value()));
        }

        auto opt_json_PointLightComponent =
            GetFromObject(json_entity, "PointLightComponent");
...
std::optional<DeserializationResult>
SceneSerializer::LoadMem(const std::vector<std::byte>& buf)
{
    ...
    DeserializationResult result(std::move(opt_scene.value()),
                                 std::move(opt_cameraStorage.value()),
                                 std::move(name));
    return result;
}
} // namespace Aether
