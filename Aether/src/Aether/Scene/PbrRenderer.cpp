...
namespace Aether {
void PbrRenderer::Render(Scene& scene, PerspectiveCamera& camera)
{
    // camera
...
void PbrRenderer::RenderSkybox(SkyboxComponent& sc,
...
void PbrRenderer::RenderObject(Scene& scene, PerspectiveCamera& camera,
                               SkyboxComponent* sc)
{
    ...
    for (const auto& [entity, mc, pmc] : view.each())
    {
        ...
        // draw
        if (!mc.model->hasBind) { mc.model->Bind(); }
        mc.model->Render();
    }
}
void PbrRenderer::Render(Scene& scene)
{
