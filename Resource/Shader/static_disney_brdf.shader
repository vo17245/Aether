...
const vec3 baseColor =vec3(.82 ,.67, .16);
...
void main()
{
    ...
    vec3 L=normalize(lightPos-v_FragmentPosition);
    vec3 V=normalize(-v_FragmentPosition);
    vec3 H=normalize(L+V);
    vec3 Cdlin = mon2lin(baseColor);
    //color=vec4(BRDF(L,V,H)+BSDF(baseColor,Roughness,dot(N,V),dot(V,H)),1);
    color=vec4(BSDF(baseColor,Roughness,dot(N,V),dot(V,H)),1);
}