#include "Render/RHI/Backend/Vulkan/Buffer.h"
...
bool Buffer::SyncCopy(const GraphicsCommandPool& commandPool, Buffer& src, Buffer& dst, size_t size, size_t srcOffset,
                      size_t dstOffset)
{
    // TODO: error handle
    ...
    commandBuffer->End();
    commandBuffer->BeginSubmit().Fence(fence).EndSubmit();
    fence.Wait();
    return true;
}
...
Buffer::Buffer(VkBuffer handle, VmaAllocation allocation, size_t size, Usage usage, Property properties,
               VmaAllocationInfo allocInfo) :
    m_Handle(handle), m_Size(size), m_Allocation(allocation), m_Usage(usage), m_Properties(properties),
...
{
    return Create(size, Buffer::PackUsages(Buffer::Usage::Storage, Buffer::Usage::TransferDst),
...
{
    return Create(size, Buffer::PackUsages(Buffer::Usage::Uniform, Buffer::Usage::TransferDst),
                  Buffer::PackProperties(Buffer::Property::DeviceLocal, Buffer::Property::HostVisible));
}
