...
{
    static constexpr int32_t value = 1 + TypeIndex<T, Ts...>::value;
};
...
struct TypeIndex<T>
{
    static constexpr int32_t value = -1; // 类型不在列表中
};
...
{
    static constexpr int32_t value = TypeIndex<T, Ts...>::value;
};
...
template <typename T>
struct TypeArrayToVariant;
template <typename... Ts>
struct TypeArrayToVariant<TypeArray<Ts...>>
{
