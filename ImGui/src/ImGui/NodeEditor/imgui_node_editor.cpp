...
//------------------------------------------------------------------------------
namespace ed = ImGui::NodeEditor::Detail;
...
//
//------------------------------------------------------------------------------
ed::EditorContext::EditorContext(const ImGui::NodeEditor::Config* config)
    : m_Config(config)
    , m_EditorActiveId(0)
    , m_IsFirstFrame(true)
    , m_IsFocused(false)
...
void ed::EditorContext::Begin(const char* id, const ImVec2& size)
{
    ...
    if (!ImRect_IsEmpty(previousVisibleRect) && previousSize != canvasSize)
    {
        ...
        if (m_Config.CanvasSizeMode == ImGui::NodeEditor::CanvasSizeMode::FitVerticalView)
        {
            height  = previousVisibleRect.GetHeight();
            width   = height * currentAspectRatio;
        }
        else if (m_Config.CanvasSizeMode == ImGui::NodeEditor::CanvasSizeMode::FitHorizontalView)
        {
            width   = previousVisibleRect.GetWidth();
            height  = width / currentAspectRatio;
        }
        else if (m_Config.CanvasSizeMode == ImGui::NodeEditor::CanvasSizeMode::CenterOnly)
        {
...
//
//------------------------------------------------------------------------------
ed::Config::Config(const ImGui::NodeEditor::Config* config)
{
    if (config)
