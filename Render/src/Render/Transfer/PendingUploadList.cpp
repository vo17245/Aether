...
PendingUploadList::CreateUploadBufferCommand(std::span<const uint8_t> data, DeviceBuffer& dstBuffer, size_t dstOffset)
{
...
{
    m_UploadBufferList.push_back(CreateUploadBufferCommand(data, dstBuffer, dstOffset));
}
void PendingUploadList::UploadInFlightBuffer(std::span<const uint8_t> data, DeviceBuffer& dstBuffer, size_t dstOffset)
{
    m_UploadInFlightBufferList.push_back(CreateUploadBufferCommand(data, dstBuffer, dstOffset));
}
} // namespace Aether